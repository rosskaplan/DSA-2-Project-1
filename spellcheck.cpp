/* 
 * Professor: Prof. Sable
 * Student: Ross Kaplan
 * Course: DSA 2
 * Semester: Fall Semester 2016
 * Program 1: Hashtable Dictionary
 * Goal: Implement a dictionary using a hashtable to quickly check if all words in a file are valid.  If a word is invalid, it'll print to the output file provided with all of the words that are over 20 characters or are not in the dictionary.  Note: Numbers mean a word is ignored and apostrophes and dashes are part of words.  All other symbols separate words.
*/
...
int main() {
    ...
    clock2 = clock();
    diff_time = double(clock2 - clock1) / CLOCKS_PER_SEC;
...
string wordToLower(string temp_word) {
    ...
    for (int i = 0; i < temp_word.length(); i++) {
        temp_word[i] = tolower(temp_word[i]); //Loop through, set all characters to lowercase
    }
return temp_word;
}
...
void spellCheckDoc(hashTable &hash_table) {
    ...
    char c;
    while (infile.get(c)) {
        if (isalnum(c) || (c == '-') || (c == '\'')) { //If character isn't a word separator
            temp_word += tolower(c);
            continue;
        }
